    ...
    function readStoredTheme() {
      try {
        ...
        if (console && typeof console.debug === "function") {
          console.debug("theme_storage_read_failed", { page: "dialogue", error: String(error && error.message || error) });
        }
        return "dark";
      }
    }

    function readRuntimeSyncTheme() {
      return themeStorageUnavailable ? getCurrentTheme() : readStoredTheme();
    }
    ...
    function updateThemeToggle(theme) {
      ...
      if (!button) return;
      button.setAttribute("aria-pressed", isLightTheme ? "true" : "false");
      button.title = isLightTheme ? "切换到暗黑模式" : "切换到明亮模式";
      const icon = button.querySelector(".material-symbols-outlined");
      if (icon) icon.textContent = isLightTheme ? "light_mode" : "dark_mode";
      const label = button.querySelector("[data-theme-label]");
      if (label) label.textContent = isLightTheme ? "明亮模式" : "暗黑模式";
    }
    ...
    function syncOutputTheme(theme) {
      const normalizedTheme = normalizeTheme(theme);
      document.querySelectorAll("[data-dialogue-theme-scope]").forEach((node) => {
        node.setAttribute("data-theme", normalizedTheme);
      });
    }

    function syncThemeToEmbeddedFrames(theme) {
      const normalizedTheme = normalizeTheme(theme);
      document.querySelectorAll("iframe").forEach((frame) => {
        try {
          ...
          injectEmbeddedConsoleStyle(doc, normalizedTheme);
        } catch (error) {
          console.warn("theme_iframe_sync_failed", {
            page: "dialogue",
            target: frame.id || frame.title || "iframe",
    ...
    function applyDialogueTheme(options) {
      const themeOptions = options || {};
      const normalizedTheme = normalizeTheme(themeOptions.theme);
      let persistResult = null;
      setDocumentTheme(normalizedTheme);
      updateThemeToggle(normalizedTheme);
      syncOutputTheme(normalizedTheme);
      if (themeOptions.persist) {
        persistResult = persistTheme(normalizedTheme);
      }
      const storageWriteFailed = persistResult && !persistResult.ok;
      const shouldNotifyStorageFailure = themeOptions.notifyOnFailure && storageWriteFailed;
      if (shouldNotifyStorageFailure) {
        toast("本次已切换，刷新后可能恢复默认", "error");
        console.warn("theme_storage_write_failed", {
          page: "dialogue",
          theme: normalizedTheme,
          source: themeOptions.source || "",
          error: persistResult.error || ""
        });
      }
      syncThemeToEmbeddedFrames(normalizedTheme);
      return normalizedTheme;
    }

    function toggleDialogueTheme() {
      return applyDialogueTheme({
        theme: getNextTheme(getCurrentTheme()),
        persist: true,
    ...
    function mapEvent(raw) {
      ...
      if (eventName === "user_text") {
        ...
        if (payload.status_kind === "commentary") return null;
        kind = "control";
    ...
    async function loadChromeStatus() {
      try {
        state.chromeStatus = await api(ROUTE_BASE + "/api/chrome-status");
      } catch (error) {
    ...
    async function openFile(path, options) {
      ...
      state.preview = {
        ...
        size: payload.size || 0,
        name: payload.name || basename(normalizedPath)
      };
      if (IS_MOBILE_MODE) openSheet("preview");
      renderPreview();
    }

    async function openDialogueLocalPath(value) {
      const target = resolveDialogueLocalFileTarget(value);
      if (!target) {
    ...
    function renderChromeStatus() {
      const button = $("chromeStatusBtn");
      if (!button) return;
      const text = $("chromeStatusText");
      const popupDisabled = $("chromePopupDisabled");
      const popupDisconnected = $("chromePopupDisconnected");
      const popupConnected = $("chromePopupConnected");
      const chrome = state.chromeStatus || {};
      const connected = !!chrome.chrome_connected;
      const serviceEnabled = chrome.service_enabled !== false && chrome.enabled !== false;
      button.classList.remove("on", "waiting", "disabled");
      if (text) text.textContent = "Chrome";
      if (serviceEnabled === false) {
        button.classList.add("disabled");
        if (text) text.textContent = "Chrome 已关闭";
        if (popupDisabled) popupDisabled.style.display = "block";
        if (popupDisconnected) popupDisconnected.style.display = "none";
        if (popupConnected) popupConnected.style.display = "none";
      } else if (connected) {
        button.classList.add("on");
        if (text) text.textContent = "Chrome 已连接";
        if (popupDisabled) popupDisabled.style.display = "none";
        if (popupDisconnected) popupDisconnected.style.display = "none";
        if (popupConnected) popupConnected.style.display = "block";
        setText("chrBrowser", (chrome.browser_info && chrome.browser_info.browser) || "Chrome");
        setText("chrTools", String(chrome.cached_tools || 0) + " 个");
        const uptimeSeconds = Number(chrome.uptime_seconds || 0);
        setText("chrUptime", uptimeSeconds > 3600
          ? (Math.floor(uptimeSeconds / 3600) + "h " + Math.floor((uptimeSeconds % 3600) / 60) + "m")
          : (Math.floor(uptimeSeconds / 60) + "m"));
      } else {
        button.classList.add("waiting");
        if (text) text.textContent = chrome.service_installed === false ? "Chrome 未安装" : "Chrome 待连接";
        if (popupDisabled) popupDisabled.style.display = "none";
        if (popupDisconnected) popupDisconnected.style.display = "block";
        if (popupConnected) popupConnected.style.display = "none";
      }
    ...
    function injectEmbeddedConsoleStyle(doc, theme) {
      ...
      style.textContent = buildWorkbenchInjectedRules(theme || getCurrentTheme()).join("");
    }
    ...
    function syncEmbeddedConsoleFrame() {
      ...
      try {
        ...
        doc.documentElement.setAttribute("data-theme", getCurrentTheme());
        injectEmbeddedConsoleStyle(doc, getCurrentTheme());
    ...
    function renderPreview() {
      ...
      if (state.preview.url) {
        if (isSameOriginPreviewUrl(state.preview.url)) {
    ...
    function renderMeScreen() {
      ...
      target.innerHTML =
        '<div class="me-stack">' +
          ...
          '</article>' +
          '<article class="me-card">' +
            '<div class="group-title">Chrome MCP</div>' +
            '<strong class="task-name">' + escapeHtml(chrome.service_status_text || "未检测") + '</strong>' +
            '<div class="task-desc">' + escapeHtml(chrome.chrome_connected ? "浏览器已连接，可继续做页面联调。" : "浏览器当前未连接。") + '</div>' +
          '</article>' +
          '<article class="me-card">' +
    ...
    function bindEvents() {
      ...
      document.querySelectorAll("[data-theme-toggle]").forEach((button) => {
        button.addEventListener("click", toggleDialogueTheme);
      });
      bindIfPresent("chromeStatusBtn", "click", (event) => {
        const popup = $("chromePopup");
        if (!popup) return;
        if (event.target && event.target.closest && event.target.closest(".chrome-status-popup")) return;
        event.stopPropagation();
        popup.classList.toggle("show");
      });
      bindIfPresent("chromeStatusBtn", "keydown", (event) => {
      ...
      bindIfPresent("chromeSettingsLink", "click", (event) => {
        event.preventDefault();
        event.stopPropagation();
        const popup = $("chromePopup");
        if (popup) popup.classList.remove("show");
        openEmbeddedView("settings", "mcp-tools");
      });
      ...
      document.addEventListener("click", (event) => {
        const popup = $("chromePopup");
        if (popup && !event.target.closest("#chromeStatusBtn")) popup.classList.remove("show");
      });
      ...
      document.addEventListener("click", (event) => {
        ...
        if (localPathLink) {
          event.preventDefault();
          openDialogueLocalPath(localPathLink.getAttribute("data-dialogue-local-path") || "")
            .catch((error) => toast(error.message, "error"));
          return;
        }
    ...
    async function init() {
      bindEvents();
      applyDialogueTheme({ theme: readStoredTheme(), persist: false, source: "initial" });
      syncDialogueDropdowns();
      await Promise.all([loadProjects(), loadConnections(), loadChromeStatus(), loadSessions()]);
      renderAll();
      if (state.activeSessionId) {
        await switchSession(state.activeSessionId);
      }
      window.addEventListener("focus", () => {
        applyDialogueTheme({ theme: readRuntimeSyncTheme(), persist: false, source: "focus" });
        loadChromeStatus().then(renderChromeStatus).catch(() => {});
        syncEmbeddedConsoleFrame();
        scheduleTaskRefresh(200);
      });
      document.addEventListener("visibilitychange", () => {
        if (document.visibilityState === "visible") {
          applyDialogueTheme({ theme: readRuntimeSyncTheme(), persist: false, source: "visibility" });
          loadChromeStatus().then(renderChromeStatus).catch(() => {});
          syncEmbeddedConsoleFrame();
        }
      ...
      window.addEventListener("storage", (event) => {
        if (event.key === THEME_STORAGE_KEY) {
          applyDialogueTheme({ theme: readStoredTheme(), persist: false, source: "storage" });
          return;
        }
        if (event.key === "opus_chrome_bridge_status") {
          loadChromeStatus().then(renderChromeStatus).catch(() => {});
        }
      });
      window.addEventListener("message", (event) => {
        if (event.origin !== window.location.origin) return;
        if (!event.data || event.data.type !== "opus-chrome-connected") return;
        loadChromeStatus().then(renderChromeStatus).catch(() => {});
      });
      state.chromeRefreshHandle = window.setInterval(() => {
        loadChromeStatus().then(renderChromeStatus).catch(() => {});
      }, 15000);
      scheduleTaskRefresh(200);
    }
