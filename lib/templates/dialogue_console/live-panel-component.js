function createDialogueLivePanelComponent(deps) {
  ...
  const AUTO_LINK_FILE_EXTENSIONS = [
    "c", "cc", "conf", "cpp", "cs", "css", "csv", "dockerfile", "env", "gif", "go", "h", "hpp",
    "html", "ico", "ini", "java", "jpeg", "jpg", "js", "json", "jsonl", "jsx", "log", "lua",
    "md", "mjs", "pdf", "png", "py", "rb", "rs", "scss", "sh", "sql", "svg", "swift", "toml",
    "ts", "tsx", "txt", "vue", "webp", "xml", "yaml", "yml"
  ].sort((left, right) => right.length - left.length || left.localeCompare(right));
  const AUTO_LINK_FILE_EXTENSION_PATTERN = AUTO_LINK_FILE_EXTENSIONS.join("|");
  const AUTO_LINK_ENTITY_PATTERN = new RegExp(
    "(https?:\\/\\/[^\\s<>\"']+)|(^|[\\s([{（【,;:，、。；：!?！？])((?:~\\/|\\.{1,2}\\/|\\/[A-Za-z0-9._@+-]+\\/|[A-Za-z0-9._@+-]+\\/)(?:[A-Za-z0-9._@+-]+\\/)*[A-Za-z0-9._@+-]+(?::\\d+(?::\\d+)?)?|[A-Za-z0-9._@+-]+\\.(?:" +
      AUTO_LINK_FILE_EXTENSION_PATTERN +
      ")(?::\\d+(?::\\d+)?)?)",
    "gi"
  );
  const {
  ...
  function renderAutoLinkedInlineText(value) {
    ...
    const entityPattern = new RegExp(AUTO_LINK_ENTITY_PATTERN);
    let output = "";
    let cursor = 0;
    let match = null;
