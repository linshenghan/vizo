...
def stream_task(orch, task_id_or_latest):
    ...
    from stream_renderer import StreamViewer
...
def _handle_agents_command(config: dict):
    ...
    try:
        ...
        sync_modules_to_claude_md(str(Path(__file__).parent))
    except Exception:
        pass
...
def _handle_copy_mode(config: dict, from_module: str):
    ...
    if final_id == new_id:
        ...
        if conflict["conflict"] and conflict["conflict_source"] == "_user":
            import shutil
            base = Path(__file__).parent / "agents" / "_user" / "default" / final_id
            if base.exists():
                shutil.rmtree(base)

    try:
        dest = copy_module(from_module, final_id)
        print(f"\n✅ 已从 {from_module} 复制到 {dest.relative_to(Path(__file__).parent)}/")
        print("\n📝 建议修改以下内容：")
...
def _handle_skeleton_mode(config: dict):
    ...
    print(f"\n✅ 骨架模块已生成: {dest.relative_to(Path(__file__).parent)}/")
...
def _detect_project_from_cwd(config_arg: str = None) -> str:
    ...
    try:
        cfg_path = Path(config_arg) if config_arg else Path(__file__).parent / "config.json"
...
def main():
    ...
    if subcmd:
        ...
        try:
            ...
            config = json.loads(Path(__file__).parent.joinpath("config.json").read_text("utf-8"))
